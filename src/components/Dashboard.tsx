...
export function Dashboard() {
  ...
  const handleGuardarCuenta = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formCuenta.banco.trim() || !formCuenta.numeroCuenta.trim() || !formCuenta.titular.trim()) return;
  ...
  const handleEliminarCuenta = async (id: string) => {
    if (!window.confirm('¿Eliminar esta cuenta bancaria de la página pública?')) return;
    ...
    <div className={`min-h-screen ${T.bgPage} text-slate-100 p-4 md:p-6 pt-[calc(1rem+env(safe-area-inset-top))] pb-[calc(1.5rem+env(safe-area-inset-bottom))] relative overflow-hidden transition-colors`}>
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className={`sticky top-[calc(env(safe-area-inset-top))] z-30 flex gap-2 mb-8 ${T.navBg} backdrop-blur-md border border-slate-800 p-1.5 rounded-2xl w-full sm:w-fit shadow-lg transition-colors overflow-x-auto`}>
          {(['inicio', 'clientes', 'finanzas', 'garantias', 'imei', 'cuentas'] as const).map((tab) => (
            <button
              key={tab}
            ...
            >
              {tab === 'inicio' ? '🏠 Inicio' : tab === 'clientes' ? '👥 Clientes' : tab === 'finanzas' ? '💰 Finanzas' : tab === 'garantias' ? '🛡️ Garantías' : tab === 'imei' ? '📡 IMEI' : '💳 Cuentas'}
            </button>
          ))}
        </div>
