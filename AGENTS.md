...
## Registro obligatorio de progreso

Al terminar cada bloque de trabajo completo (una tarea, una ronda de fixes,
antes de detenerte o de pedir confirmación al usuario) — SIEMPRE, sin
excepción — agrega una entrada nueva al final de `PROGRESS_LOG.md` en la
raíz del proyecto (créalo si no existe). No sobrescribas entradas anteriores,
solo agrega (append).

Cada entrada debe tener EXACTAMENTE este formato:

---
## [YYYY-MM-DD HH:MM] <título corto de la tarea>

### Instrucción recibida
<resumen de lo que se te pidió, en 1-3 líneas>

### Comandos ejecutados (todos, en orden)
- `<comando 1>` → <resultado/salida relevante, resumida>
- `<comando 2>` → <resultado/salida relevante, resumida>
(lista TODOS los comandos de shell/git/npm que corriste, uno por uno, en el
orden real en que los corriste — no resumas ni omitas ninguno)

### Archivos tocados (todos)
- `<archivo>` — creado / modificado / borrado / renombrado — <qué cambió,
  en una línea concreta, no genérica>
(lista TODOS los archivos, uno por uno — si son muchos, igual todos)

### Hallazgos y decisiones
<cualquier bug encontrado, decisión de diseño tomada, cosa que dejaste
pendiente a propósito, o algo que el usuario debería saber>

### Estado final
- Tests/build: <resultado exacto>
- Git: <staged / committed con hash y mensaje / working tree limpio>
---

Sé exhaustivo, no resumas de más — el usuario y otra IA (Claude en Cowork)
van a leer este archivo para saber exactamente qué hiciste, comando por
comando y archivo por archivo, sin tener que revisar el código directamente.
